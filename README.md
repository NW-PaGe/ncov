...
- The jurisdiction-focused sampling time frame of the build can be changed. It is currently set up to focus on the last year of Washington sequences, but this time frame can be altered to be shorter/longer by adjusting the `add_to_builds.sm`k and the `build.yaml` subsampling scheme.
